...
static void send_keys(void *busclientconnection)
{
    ...
    while (*ch != ((char)END))
    {
        ClientToServerEvent next_message = {.tag = PressedButton,
                                            .pressed_button = {.button = *ch}};
...
int main(void)
{
    ...
    {
        fprintf(stderr,
                "Program can not continue executing due to problem with connecting to server");
        exit(1);
    }
    ...
    if (handshake_result != NULL)
    {
        fprintf(
            stderr,
            "Program can not continue executing due to problem with handshake, problem code: %d",
            handshake_result);
        goto the_end;
    }
